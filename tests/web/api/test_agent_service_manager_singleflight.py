...
def test_remove_agent_detaches_runtime_state_when_workspace_cleanup_fails() -> None:
    """A failed workspace delete must not leave the completed task live.

    Detaching the in-memory resource owner also makes a later retry take the
    direct-directory cleanup path instead of invoking the same failed agent
    cleanup forever.
    """
    manager = AgentServiceManager()
    agent = MagicMock()
