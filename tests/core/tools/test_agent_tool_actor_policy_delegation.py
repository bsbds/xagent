...
async def _run_delegated_path(
    ...
    user: User,
    *,
    policy: MCPRuntimeAuthorizationPolicy | None,
    nested: bool,
) -> dict[str, list[str]]:
    ...
    if policy is not None:
        effective_policy = MCPRuntimeAuthorizationPolicy(
            builtin_oauth_resource_owner_key=policy.builtin_oauth_resource_owner_key,
            allowed_server_ids=frozenset({int(allowed.id)}),
        )
...
async def test_actor_policy_resolves_alice_and_excludes_unallowed_server_through_delegation(
    ...
    observations = await _run_delegated_path(
        ...
        policy=MCPRuntimeAuthorizationPolicy(
            ...
            allowed_server_ids=frozenset({999}),
        ),
        nested=False,
    )
